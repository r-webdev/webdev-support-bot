import type { MultiStepFormStep } from '../../utils/MultistepForm';
import { createMarkdownCodeBlock } from '../../utils/discordTools';
import { MINIMAL_COMPENSATION, MINIMAL_AMOUNT_OF_WORDS, POST_LIMITER_IN_HOURS } from './env';
...
const isNotShort = (str: string): boolean =>
  ...
  If your compensation is deemed unfair by the moderation team, your job posting will be removed.`.split('\n').map(item=>item.trim()).join('\n');
...
export const questions = {
    guidelines: {
      ...
      buttonDelay: 5000,
      next: (value) => value === 'ok' ? 'remote' : undefined
    },
    remote: {
      type: "button",
      body:
      ...
      next: (value:string):string => value === 'onsite' ? 'location' : 'description'
    },
    location: {
      type: "text",
      body:
