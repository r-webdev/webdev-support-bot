...
export const decay = async (
  { guild, userId }: {guild:Guild, userId?: string},
): Promise<void> => {
  try {
...
export const getTimeDiffToDecay = async (): Promise<{ diff: number; timestamp: number }> => {
  if(!lastDecay) {
...
const pointDecaySystem = async (decayData: {guild: Guild, userId?:string}): Promise<void> => {
...
export const saveLastDecay = async (timestamp: number = Date.now()): Promise<void> => {
  await upsert({
